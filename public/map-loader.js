...
document.body.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (action === "create") {
      ...
      safeRender(); // nuovo render robusto
    }
...
function render() {
  ...
  if (!container) {
    console.warn("❌ container #lesson-content non trovato. Abortito render()");
    return;
  }
...
// 🔄 render con retry
function safeRender(attempts = 10) {
  const container = document.getElementById("lesson-content");
  if (!container) {
    if (attempts > 0) {
      setTimeout(() => safeRender(attempts - 1), 150);
    } else {
      console.error("❌ #lesson-content non trovato dopo vari tentativi.");
    }
    return;
  }
  render();
